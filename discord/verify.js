...
async function main() {
  ...
  const pCh = planChannels(manifest.categories, channels, roleId, GUILD)
  ...
  pCh.update.forEach(u => drift.push(`channel drift: #${u.name} ${JSON.stringify(u.patch)}${u.overwrites?.length ? ` +${u.overwrites.length} overwrite(s)` : ''}`))
